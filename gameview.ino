  ...
  if (tetCurrent != T_NONE) 
  {
    for (int r=0; r<tetRows; r++) 
    {
      for (int c=0; c<tetCols; c++) 
      {
        ...
        {
          int minoRow = tetRow + r;
          int minoCol = tetCol + c;
          if (minoRow < ROWS_VISIBLE) 
          {
            display.drawBitmap(BOARD_X + minoCol * MINO_SIZE, BOARD_Y - minoRow * MINO_SIZE, MINO, MINO_SIZE, MINO_SIZE, WHITE);
          }
        }
      }
    }
  ...
  for (int row=0; row < ROWS_VISIBLE; row++)
  {
    if (!dontDrawFilledRows || (dontDrawFilledRows && !boardRowsToRemove[row])) 
    {
      for (int col=0; col < COLS; col++)
      {
        ...
        {
          display.drawBitmap(BOARD_X + col * MINO_SIZE, BOARD_Y - row * MINO_SIZE, MINO, MINO_SIZE, MINO_SIZE, WHITE);
        }
      }
    }
  }
