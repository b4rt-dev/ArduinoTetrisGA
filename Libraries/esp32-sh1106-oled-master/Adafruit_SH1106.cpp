...
// secondary buffer
static uint8_t secBuffer[SH1106_LCDHEIGHT * SH1106_LCDWIDTH / 8] = {
...
// return true if the page between the two buffers differ, and therefore has to be drawn
bool Adafruit_SH1106::pageDiffers(uint8_t page) { 
  
  int differs = memcmp(buffer + (page*SH1106_LCDWIDTH+SH1106_LCDWIDTH-128),secBuffer + (page*SH1106_LCDWIDTH+SH1106_LCDWIDTH-128), 128);
  if (differs == 0)
    return false;
  else
    return true;
...
void Adafruit_SH1106::display(void) {
  ...
  else
  {
    ...
    // for each of the 8 pages
  	for (int8_t i = (SH1106_LCDHEIGHT/8)-1; i >= 0; i--) //from 0 tm 7 (so 8 times)
  	{

      // check if page differs from secondary buffer
      // if yes, only then draw
      if (pageDiffers(i))
      {

        sh1106_command(0xB0 + i);    // Set page
        //sh1106_command(SH1106_SETLOWCOLUMN);    // Set lower column address //2
        //sh1106_command(SH1106_SETHIGHCOLUMN); // Set higher column address //16
        sh1106_command(0x2);
        sh1106_command(0x10);

        // for each line of 128 pixels |
        for (uint16_t j = SH1106_LCDWIDTH; j > 0; j--) // 128
        {
          Wire.beginTransmission(_i2caddr);
          WIRE_WRITE(0x40);
    ...
    memcpy(secBuffer, buffer, (SH1106_LCDWIDTH*SH1106_LCDHEIGHT/8));
